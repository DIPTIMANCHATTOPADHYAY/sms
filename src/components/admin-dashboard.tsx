...
import { LoaderCircle, ShieldBan, ShieldCheck, LogOut, Trash2, MoreVertical, LayoutGrid, AlertTriangle, ListPlus } from 'lucide-react';
import { getAdminSettings, updateAdminSettings, getAllUsers, toggleUserStatus, adminLogout, getNumberList, toggleUserAddNumberPermission } from '@/app/actions';
...
function ErrorManagementTab() {
    const { toast } = useToast();
    const [mappings, setMappings] = useState<{ reasonCode: string; customMessage: string }[]>([]);
    ...
    const handleAddMapping = () => {
        if (newMapping.reasonCode.trim() && newMapping.customMessage.trim()) {
            ...
            setMappings([...mappings, { ...newMapping, reasonCode: newMapping.reasonCode.trim() }]);
